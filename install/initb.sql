...
--
-- Table structure for table `prvt_cache`
--

CREATE TABLE IF NOT EXISTS `prvt_cache` (
  `sid` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`sid`),
  UNIQUE KEY `one` (`channel`,`account`,`code`,`date`,`hour`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
--
-- Table structure for table `publ_cache`
--

CREATE TABLE IF NOT EXISTS `publ_cache` (
...
CREATE VIEW `table_tree` AS (
select
  `data_channel`.`ch_id`   AS `ch_id`,
  `data_channel`.`ch_val`  AS `ch_val`,
  `data_account`.`ac_id`   AS `ac_id`,
  `data_account`.`ac_val`  AS `ac_val`,
  `data_plan`.`pl_id`      AS `pl_id`,
  `data_plan`.`pl_val`     AS `pl_val`,
  `data_unit`.`un_id`      AS `un_id`,
  `data_unit`.`un_val`     AS `un_val`,
  `data_unit`.`un_code_pc` AS `un_code_pc`,
  `data_unit`.`un_code_m`  AS `un_code_m`,
  group_concat(`data_idea`.`id_code_pc` separator ',') AS `grp_id_code_pc`,
  group_concat(`data_idea`.`id_code_m` separator ',') AS `grp_id_code_m`,
  group_concat(`data_idea`.`id_id` separator ',') AS `grp_id_id`
from ((`data_unit`
    left join (`data_plan`
               left join (`data_channel`
                          left join `data_account`
