...
class presentUIApi {
	...
	public function all() {
		...
		$ret = $this->db->fetchAll ( $this->sqlManager->getSql ( "/ui_present/all" ), array () );
	...
	public function row($pid) {
		...
		$ret = $this->db->fetch ( $this->sqlManager->getSql ( "/ui_present/row" ), array (
				"sid" => $pid 
		) );
		$this->cache [$cache_key] = $ret;
		return $ret;
	...
	public function getData($length, $offset = 0) {
		...
		$ret = $this->db->fetchAll ( $this->sqlManager->getSql ( "/ui_present/data" ), array (
				"length" => $length,
				"offset" => $offset 
		) );
		$this->cache [$cache_key] = $ret;
	...
	public function getDataCnt() {
		...
		$ret = $this->db->fetch ( $this->sqlManager->getSql ( "/ui_present/data_cnt" ), array () );
	...
	public function getDataByDod($dod, $length, $offset = 0) {
		...
		$ret = $this->db->fetchAll ( $this->sqlManager->getSql ( "/ui_present/data_dod" ), array (
				"dod" => $dod,
				"length" => $length,
				"offset" => $offset 
		) );
	...
	public function getDataByDodCnt($dod) {
		...
		$ret = $this->db->fetchAll( $this->sqlManager->getSql ( "/ui_present/data_dod_cnt" ), array (
				"dod" => $dod 
		) );
		if (empty ( $ret )) {
			$ret = 0;
	...
	public function getDataByUid($uid, $length, $offset = 0) {
		...
		$ret = $this->db->fetchAll ( $this->sqlManager->getSql ( "/ui_present/data_uid" ), array (
				"uid" => $uid,
				"offset" => $offset,
				"length" => $length 
		) );
	...
	public function getDataByUidCnt($uid) {
		...
		$ret = $this->db->fetch ( $this->sqlManager->getSql ( "/ui_present/data_uid_cnt" ), array (
				"uid" => $uid 
		) );
		$ret = $ret ["cnt"];
		$this->cache [$cache_key] = $ret;
	...
	private function initMetaCache() {
		$data = $this->db->fetchAll ( $this->sqlManager->getSql ( "/ui_present/meta" ), array () );
