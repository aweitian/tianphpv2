...
require_once FILE_SYSTEM_ENTRY . "/app/data/priv/article_tree/article_tree.api.php";
...
class articleModel extends privModel {
	...
	public function getInfo_tree($aid) {
	...
	public function updateTree($aid, $trd) {
		$api = new articleTreeApi ();
		if ($trd == 0) {
			return $api->rm ( $aid );
		} else {
			if (0 == $api->update ( $aid, $trd )) {
				return $api->add ( $aid, $trd );
			}
			return true;
		}
	}
	public function addTree($aid, $trd) {
		$api = new articleTreeApi ();
		return $api->add ( $aid, $trd );
	}
