...
class articleController extends privController {
	...
	public function reldocAction(pmcaiMsg $msg) {
		...
		if ($data->isTrue ()) {
			...
			$this->view->showListForReldoc ( $msg->getPmcaiUrl (), $this->priv->getUserInfo (), $data->return, $this->model->getCacheDoctor (), $page, $length );
		} else {
			echo $data->info;
			exit ();
			$this->response->showError ( $retR->info );
	...
	public function reldisAction(pmcaiMsg $msg) {
		...
		if ($data->isTrue ()) {
			...
			$this->view->showListForReldis ( $msg->getPmcaiUrl (), $this->priv->getUserInfo (), $data->return, $this->model->getCacheDisease (), $page, $length, $msg ["?dc"], $msg ["?di"], $msg ["?q"] );
		} else {
			echo $data->info;
			exit ();
			$this->response->showError ( $retR->info );
	...
	public function revreldisAction(pmcaiMsg $msg) {
		...
		if ($data->isTrue ()) {
			...
			$this->view->showListForRevReldis ( $msg->getPmcaiUrl (), $this->priv->getUserInfo (), $data->return, $this->model->getCacheDisease (), $page, $length, $msg ["?dc"], $msg ["?di"], $msg ["?q"] );
		} else {
			echo $data->info;
			exit ();
			$this->response->showError ( $retR->info );
	...
	public function revreldocAction(pmcaiMsg $msg) {
		...
		if ($data->isTrue ()) {
			...
			$this->view->showListForRevReldoc ( $msg->getPmcaiUrl (), $this->priv->getUserInfo (), $data->return, $this->model->getCacheDoctor (), $page, $length, $msg ["?do"], $msg ["?q"] );
		} else {
			echo $data->info;
			exit ();
			$this->response->showError ( $retR->info );
	...
	public function addAction(pmcaiMsg $msg) {
		...
		if ($msg->isPost ()) {
			...
			if ($retR->isTrue ()) {
				...
				if (isset ( $msg ["tree"] ) && $msg ["tree"] > 0) {
					$this->model->addTree ( $aid, $msg ["tree"] );
				}
